...
public class ElevensBoard extends Board {
    ...
    public boolean isLegal(List<Integer> selectedCards) {
        ...
        // if there are three cards.. return containsJQK
        ...
        for(int x = 0; x < selectedCards.size(); x++){
            ...
            for(int i = 0; i < selectedCards.size(); i++){
                ...
                if(firstCardIndex != secondCardIndex && 
                        outer.pointValue() + inner.pointValue() == 11)
                            return true;
            }
        }
        return false;
    ...
    private boolean containsJQK(List<Integer> selectedCards) {
        ...
        for(Integer i : selectedCards){
            if((cardAt(i).rank().equals("jack")) && 
                    (cardAt(i).pointValue() == 0)) jack = true;
            else if((cardAt(i).rank().equals("queen")) && 
                    (cardAt(i).pointValue() == 0)) queen = true;
            else if((cardAt(i).rank().equals("king")) && 
                    (cardAt(i).pointValue() == 0)) king = true;
        }
